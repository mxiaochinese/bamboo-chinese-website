...
function developmentJourney() {
  ...
  return `<div class="development-journey"><div class="journey-phases"><span><small>Giai đoạn 1</small>Nền tảng · YCT1–2</span><span><small>Giai đoạn 2</small>Mở rộng · YCT3–4</span><span><small>Giai đoạn 3</small>Tự tin · YCT5–6</span></div><div class="development-track">${stages.map((stage) => `<article class="development-stage level-${stage.level}"><div class="development-node"><span class="stage-icon">${icon(stage.icon, 19)}</span><strong>YCT${stage.level}</strong></div><p class="stage-phase">${escapeHTML(stage.phase)}</p><h3>${escapeHTML(stage.title)}</h3><p>${escapeHTML(stage.copy)}</p></article>`).join("")}</div><div class="journey-outcome"><span>${icon("leaf", 20)}</span><div><strong>Mỗi chặng là một bước trưởng thành</strong><p>Trẻ không chỉ học thêm kiến thức mà còn phát triển phản xạ, sự tự tin và khả năng sử dụng tiếng Trung ngày một tự nhiên hơn.</p></div>${htmlLink(`<span>Xem lộ trình chi tiết</span>${icon("arrow", 14)}`, "/lo-trinh", "arrow-link")}</div></div>`;
}

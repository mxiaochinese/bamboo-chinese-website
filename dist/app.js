...
function teachers(path) {
  ...
  return shell(`${pageHero("ĐỘI NGŨ GIÁO VIÊN", "Người đồng hành cùng con", "Kinh nghiệm, chứng chỉ và thế mạnh giảng dạy.", "師")}<section class="section"><div class="container"><div class="split-heading"><div><p class="eyebrow">HỒ SƠ GIÁO VIÊN</p><h2>Hiểu người đồng hành cùng con.</h2></div><p>Kinh nghiệm và thế mạnh của từng giáo viên.</p></div><div class="teacher-grid">${teachersData.map((teacher) => teacherCard(teacher)).join("")}</div></div></section><section class="section compact"><div class="container"><div class="cta-banner"><p class="eyebrow">CẦN GỢI Ý LỚP HỌC?</p><h2>Chọn điểm bắt đầu cho con.</h2>${link("Tìm lớp cho con", "/tim-lop-cho-con", "button light")}</div></div></section>`);
}

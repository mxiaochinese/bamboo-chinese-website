...
function developmentJourney() {
  ...
  return `<div class="development-journey"><div class="roadmap"><svg class="road-svg road-svg-desktop" viewBox="0 0 1200 800" role="img" aria-label="Con đường phát triển từ YCT1 đến YCT6"><path class="road-edge" d="M90 75 C300 55 350 145 320 215 C275 320 930 255 925 365 C920 480 280 390 285 505 C290 615 875 530 850 645 C830 735 655 700 600 748"/><path class="road-surface" d="M90 75 C300 55 350 145 320 215 C275 320 930 255 925 365 C920 480 280 390 285 505 C290 615 875 530 850 645 C830 735 655 700 600 748"/><path class="road-center" d="M90 75 C300 55 350 145 320 215 C275 320 930 255 925 365 C920 480 280 390 285 505 C290 615 875 530 850 645 C830 735 655 700 600 748"/></svg><svg class="road-svg road-svg-mobile" viewBox="0 0 390 1160" role="img" aria-label="Con đường phát triển từ YCT1 đến YCT6"><path class="road-edge" d="M55 70 C315 70 325 235 175 260 C35 285 55 440 230 455 C360 465 340 635 165 650 C35 665 55 835 245 845 C350 850 325 1030 195 1090"/><path class="road-surface" d="M55 70 C315 70 325 235 175 260 C35 285 55 440 230 455 C360 465 340 635 165 650 C35 665 55 835 245 845 C350 850 325 1030 195 1090"/><path class="road-center" d="M55 70 C315 70 325 235 175 260 C35 285 55 440 230 455 C360 465 340 635 165 650 C35 665 55 835 245 845 C350 850 325 1030 195 1090"/></svg><span class="road-label road-start">${icon("flag", 15)} Xuất phát</span>${stages.map((stage) => `<div class="road-stop stop-${stage.level}"><span class="road-pin${stage.level === 6 ? " goal" : ""}">${icon(stage.level === 6 ? "trophy" : "location", 18)}<strong>YCT${stage.level}</strong></span><article class="road-stage-card"><p class="stage-phase">${escapeHTML(stage.phase)}</p><h3>${escapeHTML(stage.title)}</h3><p>${escapeHTML(stage.copy)}</p></article></div>`).join("")}<span class="road-label road-finish">${icon("trophy", 15)} Đích đến</span></div><div class="journey-outcome"><span>${icon("leaf", 20)}</span><div><strong>Mỗi chặng là một bước trưởng thành</strong><p>Trẻ không chỉ học thêm kiến thức mà còn phát triển phản xạ, sự tự tin và khả năng sử dụng tiếng Trung ngày một tự nhiên hơn.</p></div>${htmlLink(`<span>Xem lộ trình chi tiết</span>${icon("arrow", 14)}`, "/lo-trinh", "arrow-link")}</div></div>`;
}
...
function coursePage(course) {
  ...
  return shell(`<section class="course-hero"><div class="container course-hero-grid"><div><div class="breadcrumb">${link("Trang chủ", "/")}<span>/</span>${link("Lộ trình học", "/lo-trinh")}<span>/</span><span>YCT${course.level}</span></div><p class="eyebrow">LỘ TRÌNH YCT${course.level}</p><h1>${escapeHTML(course.title)}</h1><p class="lede">${course.duration.totalSessions} buổi theo nhịp Học - Ôn tập - Đánh giá, phát triển Nghe - Nói - Đọc - Viết.</p><div class="course-actions">${link("Tìm lớp cho con", "/tim-lop-cho-con", "button primary")}${link("Xem toàn bộ lộ trình", "/lo-trinh", "button secondary")}</div></div><div class="course-image-frame book-frame"><img src="${courseBook(course.level)}" alt="Giáo trình YCT${course.level}" loading="eager"></div></div></section><div class="container course-facts"><div class="course-fact"><strong>${course.duration.totalSessions}</strong><span>Tổng số buổi</span></div><div class="course-fact"><strong>${course.duration.lessonSessions}</strong><span>Buổi học</span></div><div class="course-fact"><strong>${course.duration.reviewSessions}</strong><span>Buổi ôn tập</span></div><div class="course-fact"><strong>${course.duration.assessmentSessions}</strong><span>Buổi đánh giá</span></div></div>
    <section class="section compact"><div class="container"><div class="section-heading"><p class="eyebrow">CẤU TRÚC KHÓA HỌC</p><h2>Một chặng học có điểm bắt đầu, mốc củng cố và điểm nhìn lại.</h2></div><div class="milestone-grid"><article class="milestone lesson"><strong>${course.duration.lessonSessions}</strong><h3>Buổi học</h3><p>Các bài học YCT${course.level} phát triển đồng đều Nghe, Nói, Đọc và Viết qua từng chủ đề.</p></article><article class="milestone review"><strong>${course.duration.reviewSessions}</strong><h3>Buổi ôn tập</h3><p>Sau mỗi chặng, trẻ được củng cố kiến thức trước khi chuyển sang nội dung mới.</p></article><article class="milestone assessment"><strong>${course.duration.assessmentSessions}</strong><h3>Buổi đánh giá</h3><p>Gồm Nghe - Nói và Đọc - Viết ở cuối khóa.</p></article></div></div></section>
    <section class="section tint"><div class="container"><div class="section-heading"><p class="eyebrow">NỘI DUNG YCT${course.level}</p><h2>Các buổi học trong lộ trình.</h2><p class="lede">Mở từng chặng để xem chủ đề trẻ sẽ học bằng tiếng Trung và tiếng Việt.</p></div><div class="curriculum-list">${groupedCurriculum(course)}</div></div></section>
    <section class="section"><div class="container"><div class="lesson-grid"><div class="lesson-card"><div class="lesson-card-top"><span class="mini-label" style="color:var(--orange)">MỘT BUỔI HỌC</span><span class="lesson-time">${course.duration.minutesPerSession || 90} phút</span></div><h3>Học qua bốn kỹ năng.</h3><p>Trẻ được luyện Nghe, Nói, Đọc và Viết trong từng chủ đề gần gũi.</p><div class="lesson-sequence"><div class="sequence-item"><span class="sequence-dot">01</span><span>Nghe</span></div><div class="sequence-item"><span class="sequence-dot">02</span><span>Nói</span></div><div class="sequence-item"><span class="sequence-dot">03</span><span>Đọc</span></div><div class="sequence-item"><span class="sequence-dot">04</span><span>Viết</span></div></div></div><div class="lesson-copy"><div class="section-heading"><p class="eyebrow">TIẾN BỘ CỦA CON</p><h2>Nhìn thấy mốc học, ôn tập và đánh giá.</h2><p class="lede">Phụ huynh dễ dàng hình dung con đã học gì, đang củng cố nội dung nào và sẽ bước tiếp ra sao.</p></div>${link("Tìm hiểu Tiến bộ của con", "/tien-bo-cua-con", "arrow-link")}</div></div></div></section>

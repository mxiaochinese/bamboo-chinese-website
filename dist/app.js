...
function rhythmCards() {
  return `<div class="learning-flow"><article><span class="flow-icon">${icon("book", 25)}</span><div><small>01</small><h3>Học</h3><p>Tiếp nhận kiến thức mới qua các hoạt động nghe, nói, đọc và viết.</p></div></article><span class="flow-arrow">${icon("arrow", 20)}</span><article><span class="flow-icon orange">${icon("star", 25)}</span><div><small>02</small><h3>Ôn tập</h3><p>Củng cố kiến thức qua trò chơi, bài tập và hoạt động phù hợp với lứa tuổi.</p></div></article><span class="flow-arrow">${icon("arrow", 20)}</span><article><span class="flow-icon blue">${icon("chart", 25)}</span><div><small>03</small><h3>Đánh giá</h3><p>Ghi nhận sự tiến bộ của con qua từng kỹ năng để điều chỉnh nhịp học phù hợp.</p></div></article></div>`;
}
...
function finder() {
  return `<div class="finder" data-finder><div class="finder-panel"><div class="finder-progress" aria-hidden="true"><span class="progress-dot active" data-progress="1"></span><span class="progress-dot" data-progress="2"></span></div><p class="finder-question" data-finder-question>Con đã từng học tiếng Trung chưa?</p><div class="choice-grid" data-finder-choices></div><div class="finder-result" data-finder-result aria-live="polite"></div></div><div class="finder-story"><figure class="finder-scene"><img src="/assets/brand/classroom-learning-v1.png" alt="Giáo viên và trẻ cùng học trong lớp nhỏ" loading="lazy"><figcaption><strong>Đồng hành cùng con</strong><span>Từ cấp độ phù hợp đến từng bước tiến bộ</span></figcaption></figure><div class="finder-steps"><div class="mini-step"><span class="mini-step-number">01</span><div><h3>Xác định nền tảng</h3><p>Gia đình chia sẻ con đã học tiếng Trung hay chưa.</p></div></div><div class="mini-step"><span class="mini-step-number">02</span><div><h3>Nhận diện cấp độ</h3><p>Nếu con đã từng học, hãy chọn cấp độ gần nhất mà gia đình còn nhớ.</p></div></div><div class="mini-step"><span class="mini-step-number">03</span><div><h3>Đề xuất lớp học</h3><p>Bamboo kết hợp độ tuổi, nền tảng và lịch sinh hoạt để tư vấn lớp phù hợp cho con.</p></div></div></div></div></div>`;
}
...
function home() {
  return shell(`
    ...
    <section class="section finder-section"><div class="container"><div class="section-heading"><p class="eyebrow">TÌM ĐIỂM BẮT ĐẦU</p><h2>Con nên bắt đầu ở YCT nào?</h2><p class="lede">Chia sẻ độ tuổi và nền tảng tiếng Trung hiện tại của con để Bamboo đề xuất cấp độ và lớp học phù hợp.</p></div>${finder()}</div></section>
    ...
    <section class="section compact"><div class="container"><div class="cta-banner"><p class="eyebrow">BẮT ĐẦU CÙNG BAMBOO</p><h2>Chưa chắc con nên học khóa nào?</h2><p>Chia sẻ độ tuổi và nền tảng hiện tại của con để Bamboo tư vấn cấp độ phù hợp.</p>${htmlLink(`${icon("message", 16)}<span>Tìm lớp cho con</span>`, "/tim-lop-cho-con", "button light")}</div></div></section>`);
}
...
function learningPath() {
  return shell(`${pageHero("LỘ TRÌNH HỌC", "Từ YCT1 đến YCT6", "Một cách nhìn rõ ràng về các chặng học, nhịp ôn tập và những mốc đánh giá trong hành trình Bamboo.", "路")}
    <section class="section compact"><div class="container"><div class="section-heading"><p class="eyebrow">TÌM ĐIỂM BẮT ĐẦU</p><h2>Con đang ở đâu trên hành trình?</h2><p class="lede">Xác định nền tảng để gia đình dễ dàng chọn đúng chặng học tiếp theo cho con.</p></div>${finder()}</div></section>
...
function coursePage(course) {
  ...
  return shell(`<div class="course-detail level-${course.level}">
    ...
    <section class="section course-lead"><div class="container"><div class="course-section-title"><p class="eyebrow">ĐĂNG KÝ TƯ VẤN</p><h2>Nhận tư vấn về YCT${course.level}</h2><p>Chia sẻ điểm bắt đầu của con để Bamboo hỗ trợ gia đình chọn lớp phù hợp.</p></div><div class="course-lead-grid"><div class="course-lead-copy"><span>${icon("leaf", 26)}</span><h3>Một cuộc trao đổi ngắn để chọn đúng điểm bắt đầu.</h3><p>Bamboo sẽ dựa trên độ tuổi, nền tảng tiếng Trung và hình thức học mong muốn của gia đình.</p><div class="course-nav">${previous ? `<a class="course-nav-card" href="${levelPath(previous.level)}"><small>Khóa trước</small><strong>← YCT${previous.level}</strong></a>` : `<a class="course-nav-card" href="/lo-trinh"><small>Điểm bắt đầu</small><strong>Tổng quan lộ trình</strong></a>`}${next ? `<a class="course-nav-card next" href="${levelPath(next.level)}"><small>Khóa tiếp theo</small><strong>YCT${next.level} →</strong></a>` : `<a class="course-nav-card next" href="/lo-trinh"><small>Hoàn thành</small><strong>Xem toàn bộ lộ trình</strong></a>`}</div></div>${contactForm()}</div></div></section>
...
function leadPage() {
  return shell(`${pageHero("TÌM LỚP CHO CON", "Bắt đầu hành trình của con", "Chia sẻ một vài thông tin để Bamboo hiểu nhu cầu và tư vấn lớp phù hợp cho con.", "始")}<section class="section"><div class="container form-layout"><div><p class="eyebrow">THÔNG TIN CẦN THIẾT</p><h2>Cùng Bamboo chọn lớp phù hợp cho con.</h2><p class="lede">Gia đình chỉ cần chia sẻ độ tuổi, nền tảng tiếng Trung và hình thức học mong muốn.</p><div class="contact-list"><div class="contact-item"><span class="contact-icon">01</span><div><strong>Tuổi của con</strong><span>Giúp xác định cấp độ phù hợp trong hành trình dành cho trẻ 6-12 tuổi.</span></div></div><div class="contact-item"><span class="contact-icon">02</span><div><strong>Nền tảng tiếng Trung</strong><span>Cho biết con đã từng học hay chưa; nếu đã học, chọn cấp gần nhất, “Không nhớ” hoặc tự ghi thông tin khác.</span></div></div><div class="contact-item"><span class="contact-icon">03</span><div><strong>Hình thức mong muốn</strong><span>Trực tuyến, trực tiếp hoặc cần thêm tư vấn.</span></div></div></div></div>${contactForm()}</div></section>`);
}

function contactPage() {
  return shell(`${pageHero("LIÊN HỆ", "Trao đổi cùng Bamboo", "Chia sẻ nhu cầu của gia đình để Bamboo tư vấn điểm bắt đầu và lớp học phù hợp cho con.", "聯")}<section class="section"><div class="container"><div class="cta-banner"><p class="eyebrow">BƯỚC TIẾP THEO</p><h2>Tìm lớp phù hợp với nền tảng hiện tại của con.</h2><p>Bamboo sẽ trao đổi cùng gia đình về độ tuổi, nền tảng tiếng Trung và thời gian học mong muốn.</p>${link("Tìm lớp cho con", "/tim-lop-cho-con", "button light")}</div></div></section>`);
}
...
function renderFinder(root) {
  ...
  choices.addEventListener("click", (event) => {
    ...
    if (step === "prior") {
      ...
      if (value === "other") {
        result.innerHTML = `<label class="finder-other-label" for="finder-other-level">Cấp độ hoặc chương trình con từng học</label><div class="finder-other-row"><input id="finder-other-level" data-finder-other-input placeholder="Ví dụ: HSK 1 hoặc giáo trình khác"><button type="button" class="button light small" data-finder-other-submit>Tiếp tục</button></div><small>Ghi theo thông tin gia đình nhớ được.</small>`;
        result.classList.add("visible");
